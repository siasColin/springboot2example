...
function publish(obj,rootuid,rootkid){
    ...
    $.ajax( {
        data : {"infoId":articleInfoId,"parentId":kid,"toUserId":uid,"commentContent":sub_publish_,"parentId":rootkid},
        url : Common.ctxPath+"articleManage/comment",
        type : "POST",
        cache : false,
        dataType : "json",
...
function loadcomment(){
    ...
    layui.use(['flow','layer'], function() {
        ...
        var flow = layui.flow
            , layer = layui.layer;
        //请求评论信息
        var renderComment = function(page, callback){
            $.get(Common.ctxPath+'articleManage/comment/'+articleInfoId, {
                page: page || 1
            }, function(res){
                if(res.returnCode != '0'){
        ...
        flow.load({
            ...
            , done: function (page, next) { //执行下一页的回调
                renderComment(page, function (rsp) {
                    var html = dataP(rsp.data);
                    next(html, page < rsp.total);
                });
            }
        });
    });
